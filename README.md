...
1. **Add SGA to the Advanced Module List for your course in Studio**
    ...
    and add `"ev_sa"` to the "Advanced Module List" value (i.e.: the value for "Advanced Module List" should 
    be a JSON list, and that list should include `"ev_sa"`).

1. **If necessary, add SGA to installed Django apps**

    **NOTE:** This is only needed if `ev_sa` is not somehow included in `INSTALLED_APPS` already. In
    edx-platform, an app can make its way into `INSTALLED_APPS` in a few different ways, so the easiest thing
    to do is grep your edx-platform branch for 'ev_sa' and see if it appears in `INSTALLED_APPS`/`OPTIONAL_APPS`/etc.
    **All releases of edx-platform since Dogwood already include `ev_sa`, so this step is likely unnecessary.**

    If SGA is not already an installed app, add this as a top-level setting in `/edx/app/edxapp/lms.env.json` and 
    `/edx/app/edxapp/cms.env.json`:

    ```
    "ADDL_INSTALLED_APPS": ["ev_sa"],
    ```
...
1. **Add the package as a dependency (or install manually)**
    ...
    the app as it's recognized by Django is `ev_sa` with an underscore, but the repo is `edx-sga` with a dash.
...
1. **Add appropriate `ev_sa` settings**

    - In both `cms/envs/common.py` and `lms/envs/common.py`, 
    add `'ev_sa'` to `OPTIONAL_APPS`
    - In `cms/envs/common.py`, add `'ev_sa'` to `ADVANCED_COMPONENT_TYPES`
...
<vertical display_name="Staff Graded Assignment">
    <ev_sa url_name="sga_example" weight="10.0" display_name="SGA Example" points="100.0" showanswer="past_due" solution="solution text" />
</vertical>
```
...
```sh
python manage.py lms --settings=test test ev_sa.tests.integration_tests
```
...
```sh
coverage run --source ev_sa manage.py lms --settings=test test ev_sa.tests.integration_tests
coverage report -m
```
