...
from ev_sa.tests.common import TempfileMixin, DummyResource
...
class StaffGradedAssignmentMockedTests(TempfileMixin):
    ...
    def make_xblock(self, display_name=None, **kwargs):
        ...
        """
        from ev_sa.sga import StaffAssignmentXBlock as cls
    ...
    @mock.patch('ev_sa.sga._resource', DummyResource)
    @mock.patch('ev_sa.sga.render_template')
    @mock.patch('ev_sa.sga.Fragment')
    def test_student_view(self, fragment, render_template):
        ...
        with mock.patch(
            'ev_sa.sga.StaffAssignmentXBlock.get_submission',
            return_value={}
        ), mock.patch(
            'ev_sa.sga.StaffAssignmentXBlock.student_state',
            return_value={
                'uploaded': None,
                'annotated': None,
                'upload_allowed': True,
            ...
            fragment.add_css.assert_called_once_with(
                DummyResource("static/css/ev_sa.css"))
            fragment.initialize_js.assert_called_once_with(
                "StaffAssignmentXBlock")

    @mock.patch('ev_sa.sga._resource', DummyResource)
    @mock.patch('ev_sa.sga.StaffAssignmentXBlock.upload_allowed')
    @mock.patch('ev_sa.sga.StaffAssignmentXBlock.get_score')
    @mock.patch('ev_sa.sga.render_template')
    @mock.patch('ev_sa.sga.Fragment')
    def test_student_view_with_score(self, fragment, render_template, get_score, upload_allowed):
        ...
        with self.dummy_upload('foo.txt') as (upload, _):
            with mock.patch(
                ...
                'ev_sa.sga.StaffAssignmentXBlock.student_state', return_value={}
            ), mock.patch(
                'ev_sa.sga.StaffAssignmentXBlock.get_or_create_student_module',
    ...
    def test_studio_view(self):
        ...
        """
        with mock.patch('ev_sa.sga.StudioEditableXBlockMixin.studio_view') as studio_view_mock:
            block = self.make_xblock()
            block.studio_view()
    ...
    @mock.patch('ev_sa.sga.StaffAssignmentXBlock.get_student_item_dict')
    @mock.patch('ev_sa.sga.StaffAssignmentXBlock.upload_allowed')
    @mock.patch('ev_sa.sga.get_sha1')
    def test_upload_download_assignment(self, get_sha1, upload_allowed, get_student_item_dict):
        ...
        with self.dummy_upload(file_name) as (upload, expected):
            with mock.patch('submissions.api.create_submission') as mocked_create_submission, mock.patch(
                "ev_sa.sga.StaffAssignmentXBlock.file_storage_path",
    ...
    def test_finalize_uploaded_assignment(
        ...
        with mock.patch(
            'ev_sa.sga.Submission.objects.get', return_value=fake_submission_object
        ), mock.patch(
            'ev_sa.sga.StaffAssignmentXBlock.get_submission', return_value=fake_submission_data
        ), mock.patch(
            'ev_sa.sga.StaffAssignmentXBlock.student_state', return_value={}
        ):
    ...
    @mock.patch('ev_sa.sga.StaffAssignmentXBlock.get_student_module')
    @mock.patch('ev_sa.sga.StaffAssignmentXBlock.is_course_staff')
    @mock.patch('ev_sa.sga.get_sha1')
    def test_staff_upload_download_annotated(self, get_sha1, is_course_staff, get_student_module):
        ...
        with self.dummy_upload(file_name) as (upload, expected), mock.patch(
            "ev_sa.sga.StaffAssignmentXBlock.staff_grading_data",
            return_value={}
    ...
    def test_upload_allowed(self, past_due, score, is_finalized_submission, expected_value):
        ...
        block = self.make_xblock()
        with mock.patch(
            "ev_sa.sga.StaffAssignmentXBlock.past_due",
            return_value=past_due
        ), mock.patch(
            "ev_sa.sga.StaffAssignmentXBlock.get_score",
            return_value=score
        ), mock.patch(
            "ev_sa.sga.is_finalized_submission",
            return_value=is_finalized_submission
        ):
            assert block.upload_allowed(submission_data={}) is expected_value

    @mock.patch('ev_sa.sga.zip_student_submissions')
    @mock.patch('ev_sa.sga.StaffAssignmentXBlock.get_sorted_submissions')
    ...
    def test_prepare_download_submissions(
        ...
        with mock.patch(
            "ev_sa.sga.StaffAssignmentXBlock.is_zip_file_available",
            return_value=is_zip_file_available
        ), mock.patch(
            'ev_sa.sga.StaffAssignmentXBlock.get_real_user',
            return_value=self.staff
        ), mock.patch(
            'ev_sa.utils.default_storage.modified_time',
    ...
    def test_prepare_download_submissions_task_called(
        ...
        with mock.patch(
            ...
            'ev_sa.sga.default_storage.modified_time',
            return_value=datetime.datetime.now()
        ):
            response = block.prepare_download_submissions(None)
            response_body = json.loads(response.body)
    ...
    def test_download_submissions(self, is_course_staff):
        ...
        with mock.patch(
            "ev_sa.sga.get_zip_file_path", return_value=path
        ), mock.patch(
            'ev_sa.sga.StaffAssignmentXBlock.get_real_user',
            return_value=self.staff
        ), mock.patch(
            "ev_sa.sga.get_zip_file_name", return_value=filename
        ):
    ...
    def test_clear_student_state(self):
        ...
        with self.dummy_file_in_storage(uploaded_file_path) as file_path:
            with mock.patch(
                "ev_sa.sga.submissions_api.get_submissions",
                return_value=[fake_submission]
            ) as mocked_get_submissions, mock.patch(
                "ev_sa.sga.submissions_api.reset_score"
