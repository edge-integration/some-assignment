...
from ev_sa.constants import ShowAnswer
from ev_sa.sga import StaffAssignmentXBlock
from ev_sa.tests.common import (
    TempfileMixin,
    DummyResource,
    get_sha1,
    is_near_now,
...
@ddt
class StaffAssignmentXBlockTests(TempfileMixin, ModuleStoreTestCase):
    """
    Create a SGA block with mock data.
    """
    @classmethod
    def setUpClass(cls):
        super(StaffAssignmentXBlockTests, cls).setUpClass()
        cls.set_up_temp_directory()

    @classmethod
    def tearDownClass(cls):
        super(StaffAssignmentXBlockTests, cls).tearDownClass()
    ...
    def setUp(self):
        ...
        """
        super(StaffAssignmentXBlockTests, self).setUp()
    ...
    def test_enter_grade_fail(self, grade):
        ...
        block = self.make_one()
        fred = self.make_student(block, "fred5", filename='foo.txt')
        with mock.patch('ev_sa.sga.log') as mocked_log:
            block.enter_grade(
                mock.Mock(
                    params={
                        'module_id': fred['module'].id,
    ...
    def test_showanswer(self, is_answer_available):
        ...
        with mock.patch(
            'ev_sa.sga.StaffAssignmentXBlock.answer_available',
    ...
    def make_test_vertical(self, solution_attribute=None, solution_element=None):
        ...
        return (
            """<vertical display_name="SGA Unit">
              <ev_sa url_name="ev_sa" xblock-family="xblock.v1" display_name="SGA Test 1" {solution_attribute}>
                {solution_element}
              </ev_sa>
